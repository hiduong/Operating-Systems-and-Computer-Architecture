...
int show_bmp(char *p, int startRow, int startCol)
{
   ...
   for (i=startRow; i<(h+startRow); i += 1){
     ...
     for (j=startCol; j<(startCol+w); j+=1){
       setpix(j,i);
       pp += 3;    // go to next pixel
     }
     e -= r2;
   }


   for (i=startRow; i<(h+startRow)/counter; i += 1){
     pp = p;
     for (j=startCol; j<(startCol+w)/counter; j+=1){
       b = *pp; g = *(pp+1); r = *(pp+2); //brg values
       pixel = (b<<16) + (g<<8) + r; //pixel value
       if(counter == 1){ //write to frame buffer
	   fb[i*640 + j] = pixel;
	 }
	 else if(counter == 2){
	   fb[i*640 + j + 640*30 + 35] = pixel;
	 }
	 else if(counter == 3){
	   fb[i*640 + j + 640*105 + 125] = pixel;
	 }
	 else if(counter == 4){
	   fb[i*640 + j + 640*130 + 180] = pixel;
	 }
         pp += 3 * counter;    // go to next pixel
     }
     p -= r2 * counter; // to next row
   }
